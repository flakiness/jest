...
| 3 | Multiple environments | ✅ | One `FK.Environment` per unique `ProjectConfig.id` (captured in `onTestResult` via `test.context.config`). Environment `name` is the project's `displayName` (`"jest"` fallback). If two projects share a displayName, subsequent ones are suffixed `" (2)"`, `" (3)"`, etc. so every environment is distinctly labeled. Each attempt's `environmentIdx` points at its file's project. |
